...
function fetch_user($user_id = null)
{
	...
	global $link;
	$status = "running";
	$sql = "SELECT * FROM trans WHERE status = '$status'";
	$query = mysqli_query($link, $sql);
...
if (isset($all_inv)) {
	...
	foreach ($all_inv as $value) {
		...
		$plan = $value['plan'];
		$usd_amt = floatval($value['usd_amt']);
		$count = $value['count'] + 1;
		$plan = $value['plan'];
		$intRate = $value['interest'];
		$days = $value['days'];
		$date = date('d M Y');
		$time = date('H:m:s');
	...
	if (($query2 === true) OR ($query4 === true)) {
		$to = $support_email;
		$name = "Finance";
		$subject = "Automated Profit Report";
		$body = "Profit Added for " . $total . " User(s)";
		// $send = send_mail($to, $name, $subject, $body);
		...
		$name = "Finance";
		$subject = "Automated Profit Report";
		$body = "Profit Failed to Add! This could mean there were no Active Investment. If there are active Investment, report this issue to your Developer";
		// $send = send_mail($to, $name, $subject, $body);
	}
